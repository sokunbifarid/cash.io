...
func set_active_skin(skin: String) -> void:
	if skin != "":
		...
		active_skin = skin
		selected_skin_texture_button.unselect_skin(active_skin)
...
func populate_skins(avatars: Array) -> void:
	...
	for i in avatars:
		...
		if active_skin == i:
			skin.select_skin(active_skin)
		else:
			skin.unselect_skin(active_skin)
...
func _on_skin_button_pressed(sender_skin_id: String) -> void:
	active_skin = sender_skin_id
	for i in profile_skin_panel_grid_container.get_children():
		if active_skin == i.skin_id:
			i.select_skin(active_skin)
		else:
			i.unselect_skin(active_skin)
