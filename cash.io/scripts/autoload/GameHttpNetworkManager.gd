...
func map_networked_payload(payload: Dictionary) -> void:
	if payload.has("topic"):
		var payload_topic: int = payload.topic
		match payload_topic:
			ms_topic.GET_ROOMS:
			...
			ms_topic.GET_SHOP_CATALOG:
				append_shop_catalog(payload)
			ms_topic.BUY_CATALOG_ITEM:
...
func append_room_joined_data(payload: Dictionary) -> void:
	...
	if payload.has("initial_payload"):
		...
		if payload.initial_payload.has("powerups"):
			print("powerups payload initial: ", payload.initial_payload.powerups)
		...
		if payload.snapshot_payload.has("spawned_players"):
			if payload.snapshot_payload.spawned_players != null:
				if payload.snapshot_payload.spawned_players.size() > 0:
					...
					populate_player_list(payload.snapshot_payload, true)
...
func populate_player_list(payload: Dictionary, new_player_joining: bool = false) -> void:
	...
	if temp_player_list.size() > 0:
		...
		if populate_player_list_basket_players.size() > 0:
			SignalManager.emit_load_players_on_join_match_signal(populate_player_list_basket_players, room_bound, new_player_joining)
...
func send_use_powerup(id: String) -> void:
	...
	powerup_to_use.powerup_id = id
	powerup_to_use.user_id = get_current_player_id()
